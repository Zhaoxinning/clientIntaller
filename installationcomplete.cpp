    ...
    isInstallSucceed(isInstallSucceed)
{
    ...
    if (isInstallSucceed) {
        ui->labelTitleComplete->setText(tr("   %1 %2 已经完成安装").arg(cnc::exeName,cnc::versions));
        ...
        ui->labelTitleComplete->setText(tr("%1 %2 安装失败").arg(cnc::exeName,cnc::versions));
...
void InstallationComplete::addLinkToDeskTop(const QString &filename)
{
    ...
    //writableLocation:返回类型为文件所属的所有目录。
    //QStandardPaths::DesktopLocation:枚举。此枚举中的某些值表示用户配置。这样的枚举值将在不同的应用程序中返回相同的路径，可以使用它们与其他应用程序共享数据。
    ...
    //link():创建一个名为linkName的链接，该链接指向当前有fileName()指定的文件。
